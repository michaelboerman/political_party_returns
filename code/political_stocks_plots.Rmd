...
This is a simple code for showing how stocks and bonds do when the presidential party is Democratic or Republican. More so, it's a *very* simple showcase of my tidyverse toolbox!
...
data <-
  ...
  select(party, year, bonds, stocks, political_colors) %>% 
...
This whole chunk is really one call. I had been using pmap with several inputs, but using `pivot_longer` above eliminated this need. I still use map to loop over stocks and bonds, which ensures that all are on one plot.
...
map(
  ...
  function(asset) {
   ...
   plot_title <- "Annual Returns by Political Party and Asset Class"
   ...
   plot_caption <- paste0(
    ...
    data %>% 
      ...
      scale_x_discrete(labels = c("Stocks", "Bonds")) +
      theme_minimal() +
      labs(
        title   = plot_title,
      ...
      ggsave(filename = here("plots/asset_returns.png"))
  }
)
```
